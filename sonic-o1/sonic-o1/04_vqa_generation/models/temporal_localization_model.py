...
class TemporalLocalizationModel(BaseGeminiClient):
    ...
    def _generate_temporal_questions_with_retry(self,
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if result:
                    logger.info(f"✓ Successfully generated temporal questions for segment {seg_num} on attempt {attempt + 1}")
                    return result
                else:
        ...
        logger.error(f"✗ Failed to generate temporal questions for segment {seg_num} after {self.max_retries} attempts. Last error: {last_error}")
        return None
    ...
    def _generate_temporal_questions_for_segment(self,
        ...
        try:
            ...
            print(f"🔍 Segment {seg_num}: start={segment_info['start']}, end={segment_info['end']}, duration={segment_info['end']-segment_info['start']}")
            ...
            if self.judge_enabled:
                ...
                try:
                    ...
                    logger.info(f"🔍 Judge video exists: {Path(video_path).exists() if video_path else False}")
                    ...
                    if video_path and Path(video_path).exists():
                        ...
                        if frame_paths:
                            ...
                            logger.info(f"✓ {validation_stats['valid']}/{validation_stats['total']} valid, {validation_stats['fixed']} fixed")
                except Exception as e:
                ...
                if segment_start > 0:  # Not first segment (can't detect for segment 0)
                    ...
                    if start_s > segment_end or end_s > segment_end:
                        ...
                        if 0 <= test_start <= segment_duration and 0 <= test_end <= segment_duration:
                            logger.warning(f"Q{i+1}: Double-converted timestamps [{start_s}, {end_s}] → fixing to relative [{test_start:.1f}, {test_end:.1f}]")
                            start_s = test_start
                            end_s = test_end
                    ...
                    # BUT need to convert to relative for bounds checking
                        ...
                        logger.warning(f"Q{i+1}: Absolute timestamps detected [{start_s}, {end_s}] → converting to relative")
