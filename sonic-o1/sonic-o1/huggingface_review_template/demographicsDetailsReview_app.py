...
def load_topic(topic, current_state):
    ...
    try:
        ...
        return gr.update(), f"❌ Error loading JSON: {e}", "<em>Error</em>", "<em>Error</em>", "—", "—", "", "", "", "", "", "", current_state
...
def move(delta, race_text, gender_text, age_text, lang_text, current_state):
    ...
    if not current_state or not current_state.get("items"):
        return "<em>Load a topic first.</em>", "<em>No caption.</em>", "—", "—", "", "", "", "", "", "", current_state
...
with gr.Blocks(title="Demographics Review Tool", theme=gr.themes.Soft()) as demo:
    ...
    with gr.Column(visible=False) as app_view:
        ...
        gr.Markdown("💡 **Tip:** Changes auto-save when you navigate. Original AI annotations are preserved.")
