...
def load_task_topic(task, topic, current_state):
    ...
    print(f"🔵 Loading task '{task}' topic '{topic}' for session {current_state['session_id']}")
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        return gr.update(), f"❌ Error loading JSON: {e}", *empty_display(), current_state
...
def jump_to_question(question_num, current_state, demographics_json, *edit_fields):
    ...
    try:
        ...
        if target_idx < 0 or target_idx >= len(current_state["items"]):
            return f"❌ Invalid question number. Please enter a number between 1 and {len(current_state['items'])}.", *show_current(current_state), current_state
        ...
        return f"✅ Jumped to question {question_num}", *show_current(current_state), current_state
        
    except ValueError:
        return "❌ Please enter a valid number.", *show_current(current_state), current_state
...
with gr.Blocks(title="VQA Review & Edit Tool", theme=gr.themes.Soft()) as demo:
    ...
    with gr.Column() as login_view:
        passcode_input = gr.Textbox(label="Enter Passcode", type="password", placeholder="••••••••")
    ...
    with gr.Column(visible=False) as app_view:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                segment_jump_btn = gr.Button("⏩ Jump to Segment", visible=False, variant="secondary")
